...
class Gps extends CI_Controller {
    ...
    public function gps_dashboard()
    {
        ...
        if($stat==1){//admin
            ...
            //$data['vendorStock'] = $this->VendorModel->vendorDashboard()->result_object();
            $data['serviceableStock']    = $this->GpsModel->serviceableDashboard()->result_object();
            $data['unserviceableStock']    = $this->GpsModel->unserviceableDashboard()->result_object();
            $data['data']    	 = $this->GpsModel->getGpsList()->result_object();
            $data['content'] 	 = 'user-modul/gps/gps_dashboard';
            $this->load->view('user-modul/template',$data);
        }else{ //user
            $this->session->sess_destroy();
            redirect('login','refresh');
