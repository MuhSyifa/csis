...
header("Content-Disposition: attachment; filename=daftar_data_GPS_Type". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,1,'GPS Type',$header);  // Set Nama kolom
$worksheet1->set_column(0,1,20); // Set lebar kolom 
$worksheet1->write_string(0,2,'Condition',$header);  // Set Nama kolom
$worksheet1->set_column(0,2,50); // Set lebar kolom
$worksheet1->write_string(0,3,'Status',$header);  // Set Nama kolom
...
$row = 1;
foreach ($sql1 as $key) {
    $worksheet1->write_string($row,0,  $key->gps_type_id ,$content);
    $worksheet1->write_string($row,1,  $key->gps_type_name ,$content);
    $worksheet1->write_string($row,2,  $key->gps_type_condition ,$content);
    $worksheet1->write_string($row,3,  $key->gps_type_status ,$content);
    $row++;
}
