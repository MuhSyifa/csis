...
header("Content-Disposition: attachment; filename=daftar_data_GPS". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,1,'GPS IMEI Number',$header);  // Set Nama kolom
$worksheet1->set_column(0,1,20); // Set lebar kolom 
$worksheet1->write_string(0,2,'GPS Received By',$header);  // Set Nama kolom
$worksheet1->set_column(0,2,15); // Set lebar kolom
$worksheet1->write_string(0,3,'Vendor',$header);  // Set Nama kolom
...
foreach ($data as $key) {
    ...
    $worksheet1->write_string($row,1,  $key->gps_imei_number ,$content);
    $worksheet1->write_string($row,2,  $key->gps_received_by ,$content);
    $worksheet1->write_string($row,3,  $key->vendor_id ,$content);
    $row++;
}
