...
header("Content-Disposition: attachment; filename=daftar_data_Vendor". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,1,'Nama Vendor',$header);  // Set Nama kolom
$worksheet1->set_column(0,1,20); // Set lebar kolom 
$worksheet1->write_string(0,2,'Type Vendor',$header);  // Set Nama kolom
...
$row = 1;
foreach ($data as $key) {
    $worksheet1->write_string($row,0,  $key->vendor_id ,$content);
    $worksheet1->write_string($row,1,  $key->vendor_name ,$content);
    $worksheet1->write_string($row,2,  $key->vendor_type ,$content);
    $row++;
}
