...
header("Content-Disposition: attachment; filename=daftar_data_GSM_Activated". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,1,'GSM Number',$header);  // Set Nama kolom
$worksheet1->set_column(0,1,20); // Set lebar kolom 
$worksheet1->write_string(0,2,'GSM IMSI Number',$header);  // Set Nama kolom
$worksheet1->set_column(0,2,50); // Set lebar kolom
$worksheet1->write_string(0,3,'GSM ICCID Number',$header);  // Set Nama kolom
$worksheet1->set_column(0,3,15); // Set lebar kolom
$worksheet1->write_string(0,4,'GSM Activated Date',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,15); // Set lebar kolom
$worksheet1->write_string(0,5,'GSM Activated By',$header);  // Set Nama kolom
...
foreach ($data as $key) {
    ...
    $worksheet1->write_string($row,4,  $key->gsm_activated_date ,$content);
    $worksheet1->write_string($row,5,  $key->gsm_activated_by ,$content);
    $row++;
}
