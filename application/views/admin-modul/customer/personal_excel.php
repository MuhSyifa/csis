...
header("Content-Disposition: attachment; filename=daftar_data_Customer_Personal". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,3,'Full Name',$header);  // Set Nama kolom
$worksheet1->set_column(0,3,15); // Set lebar kolom
$worksheet1->write_string(0,4,'Short Name',$header);  // Set Nama kolom
$worksheet1->set_column(0,4,15); // Set lebar kolom
$worksheet1->write_string(0,5,'Gender',$header);  // Set Nama kolom
$worksheet1->set_column(0,5,15); // Set lebar kolom
$worksheet1->write_string(0,6,'Religion',$header);  // Set Nama kolom
$worksheet1->set_column(0,6,15); // Set lebar kolom
$worksheet1->write_string(0,7,'Email',$header);  // Set Nama kolom
$worksheet1->set_column(0,7,15); // Set lebar kolom
$worksheet1->write_string(0,8,'Phone',$header);  // Set Nama kolom
$worksheet1->set_column(0,8,15); // Set lebar kolom
$worksheet1->write_string(0,9,'Mobile Phone',$header);  // Set Nama kolom
$worksheet1->set_column(0,9,15); // Set lebar kolom
$worksheet1->write_string(0,10,'Address',$header);  // Set Nama kolom
$worksheet1->set_column(0,10,15); // Set lebar kolom
$worksheet1->write_string(0,11,'Start Contract',$header);  // Set Nama kolom
$worksheet1->set_column(0,11,15); // Set lebar kolom
$worksheet1->write_string(0,12,'End Contract',$header);  // Set Nama kolom
$worksheet1->set_column(0,12,15); // Set lebar kolom
$worksheet1->write_string(0,13,'Maintenance Contract',$header);  // Set Nama kolom
...
foreach ($personal_excel as $key) {
    ...
    $worksheet1->write_string($row,3,  $key->cust_full_name ,$content);
    $worksheet1->write_string($row,4,  $key->cust_short_name ,$content);
    $worksheet1->write_string($row,5,  $key->cust_gender ,$content);
    $worksheet1->write_string($row,6,  $key->cust_religion_id ,$content);
    $worksheet1->write_string($row,7,  $key->cust_personal_email ,$content);
    $worksheet1->write_string($row,8,  $key->cust_personal_phone ,$content);
    $worksheet1->write_string($row,9,  $key->cust_personal_mobile_phone ,$content);
    $worksheet1->write_string($row,10,  $key->cust_personal_address ,$content);
    $worksheet1->write_string($row,11,  $key->cust_start_date_contract ,$content);
    $worksheet1->write_string($row,12,  $key->cust_end_date_contract ,$content);
    $worksheet1->write_string($row,13,  $key->cust_maintenance_contract ,$content);
    $row++;
}
