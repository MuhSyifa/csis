...
header("Content-Disposition: attachment; filename=daftar_data_Vehicle_Installed". date('dmY_His').".xls" );
header("Expires: 0");
...
$worksheet1->write_string(0,5,'Install Date',$header);  // Set Nama kolom
...
foreach ($vehicle as $key) {
    ...
    $worksheet1->write_string($row,5,  $key->veh_install_date ,$content);
 ...
 */<?php
header("Content-type: application/vnd.ms-excel");
header("Content-Disposition: attachment; filename=daftar_data_Vehicle". date('dmY_His').".xls" );
header("Expires: 0");
header("Cache-Control: must-revalidate, post-check=0,pre-check=0");
header("Pragma: public");
...
foreach ($vehicle as $key) {
    ...
    $worksheet1->write_string($row,1,  $key->veh_name ,$content);
    $worksheet1->write_string($row,2,  $key->veh_number_police ,$content);
    $worksheet1->write_string($row,3,  $key->veh_number ,$content);
    $worksheet1->write_string($row,4,  $key->veh_type ,$content);
    $row++;
}
