                ...
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <div class="responsive-table">
                            <table id="datatables-example-cust-company" class="table table-striped table-bordered" cellspacing="0" width="100%">
                            <thead>
                                <tr>
                ...
                <div class="modal-header">
                    ...
                    <div class="tab-content">
                      <div class="tab-pane active" id="profile">-->
                        ...
                        <div class="form-group"> 
                            <label class="control-label col-md-3">Company Business Type</label>
                            <div class="col-md-9">
        ...
        <div class="modal-content">
            ...
            <form method="post" action="<?php echo base_url('customer/add_pic'); ?>" class="form-horizontal">
                <div class="modal-body form">
                    ...
                    <input type="hidden" value="" name="cust_code_add"/>
                    <input type="hidden" value="" name="cust_company_name_add"/>
                    <input type="hidden" value="" name="cust_business_type_id_add"/>
                    <input type="hidden" value="" name="cust_company_phone_add"/>
                    <input type="hidden" value="" name="cust_company_email_add"/>
                    <input type="hidden" value="" name="cust_company_address_add"/>
                    <input type="hidden" value="" name="cust_start_date_contract_add"/>
                    <input type="hidden" value="" name="cust_end_date_contract_add"/>
...
<script type="text/javascript">
	...
	// when button search on click
	$("#btnSearch").on("click", function(){
		var valCompany = $('#fcompany').val();
		var valVehicle = $('#fvehicle').val();
		var valImei = $('#fimei').val();
		var valSimCardNo = $('#fsimcardno').val();
		var strJson = '{"company":"'+ valCompany +'","vehicle":"'+ valVehicle +'","imei":"'+ valImei +'","gsmNumber":"'+ valSimCardNo +'"}';
		var url = globalTableUrl + '&params='+strJson;
		$('#advancedDataTable').dataTable().fnDestroy();
		refreshTable(url);
	});
    ...
    {
        $('.help-block').empty(); // clear error string
        $('.modal-title').text('Form Add Contract Person'); // Set Title to Bootstrap modal title
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('[name="cust_code_add"]').val(data.cust_code);
                $('[name="cust_company_name_add"]').val(data.cust_company_name);
                $('[name="cust_business_type_id_add"]').val(data.cust_business_type);
                $('[name="cust_company_phone_add"]').val(data.cust_company_phone);
                $('[name="cust_company_email_add"]').val(data.cust_company_email);
                $('[name="cust_company_address_add"]').val(data.cust_company_address);
                $('[name="cust_start_date_contract_add"]').val(data.cust_start_date_contract);
                $('[name="cust_end_date_contract_add"]').val(data.cust_end_date_contract);
    ...
    {
        $('.help-block').empty(); // clear error string
        $('.modal-title').text('Form Edit Customer Company'); // Set Title to Bootstrap modal title
    ...
    {
        $('.help-block').empty();
        $('.modal-title').text('Detail Customer Company'); // Set Title to Bootstrap modal title 
            //Ajax Load data from ajax
        $.ajax({
